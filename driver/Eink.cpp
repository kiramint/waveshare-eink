...
void Eink::Touch::registerCallback(function<void(vector<EinkTouchPoint>)> callback)
{
    _callback = callback;
}

void Eink::Touch::scanLoop()
{
        ...
        try
        {
            ...
            if (DEV_Digital_Read(INT) == 0)
            {
                ...
                if (Dev_Now->TouchpointFlag)
                {
                    ...
                    {
                        bool pressed = true;
                        if (Dev_Now->TouchpointFlag == 0x00)
                        {
                            pressed = false;
                        }
...
void Eink::Display::updateImage(cv::Mat sourceImage, Eink::Update mode)
{
    // Update last image
...
void Eink::Display::updateImageAuto(cv::Mat sourceImage)
{
    if (lastImage.empty())
    {
...
int Eink::Display::imageDiffs(cv::Mat thisImage)
{
...
u_int8_t* Eink::Display::convertImage(cv::Mat sourceImage)
{
    // Resize image
